...
Player Board::CheckWinner() const
{
    ...
    for (int i{}; i < 3; ++i)
    {
        if (player = check(i, 0, 0, 1, m_Board); player != Player::None)
            return player;

        if (player = check(0, i, 1, 0, m_Board); player != Player::None)
            return player;
    }

    // Verifica diagonais
    if (player = check(0, 0, 1, 1, m_Board); player != Player::None)
        return player;

    if (player = check(0, 2, 1, -1, m_Board); player != Player::None)
        return player;
